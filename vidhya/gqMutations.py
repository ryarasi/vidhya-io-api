...
class createGoogleToken(graphene.Mutation):
    ...
    def mutate(self, info, input=None):
        ...
        if (User.objects.filter(email=input.email).exists() == False):
            user_instance = User(email=input.email, first_name=input.first_name, last_name=input.last_name,
                                 name=input.first_name + ' ' + input.last_name, username=input.username, searchField=searchField)
            user_instance.save()
        else:
