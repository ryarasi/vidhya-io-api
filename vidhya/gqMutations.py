...
class CreateInstitution(graphene.Mutation):
    ...
    def mutate(root, info, input=None):
        ...
        searchField += str(input.dob) if input.dob is not None else str(
            timezone.now())
...
class UpdateInstitution(graphene.Mutation):
    ...
    def mutate(root, info, id, input=None):
        ...
        if institution_instance:
            ...
            searchField += str(institution_instance.dob) if institution_instance.dob is not None else str(
                timezone.now())
...
class GenerateEmailOTP(graphene.Mutation):
    ...
    def check_email_verified(email=None):
        ...
        try:
            ...
            verified_email = EmailOTP.objects.get(
    ...
    def mutate(root, info, input=None):
        ...
        if user_instance:
            ...
            if user_instance.institution:
                ...
                searchField += user_instance.institution.designation if user_instance.institution.designation is not None else ""
                searchField += user_instance.institution.participant if user_instance.institution.participant is not None else ""
...
class CreateAnnouncement(graphene.Mutation):
    ...
    def mutate(root, info, input=None):
        ...
        announcements_modified(announcement_instance)
...
class MarkAnnouncementsSeen(graphene.Mutation):
    ...
    def mutate(root, info,):
        ...
        # Invalidating user specific announcements
        user_announcements_modified(current_user)
...
class ClapProject(graphene.Mutation):
    ...
    def mutate(root, info, id):
        ...
        if project:
            if current_user.id:
                user_already_clapped = ProjectClap.objects.filter(
                    user_id=current_user.id, project_id=project.id).exists()
        ...
        if input.rubric:
            for criterion in input.rubric:
                criterion_instance = Criterion(
                    exercise_id=exercise_instance.id, description=criterion.description, points=criterion.points)
                criterion_instance.save()

        # Updating the points on the chapter
        CreateChapter.update_points(input.chapter_id)

        exercise_key_instance = ExerciseKey(exercise=exercise_instance, course_id=input.course_id, chapter_id=input.chapter_id, valid_option=input.valid_option,
...
class UpdateExercise(graphene.Mutation):
    ...
    def mutate(root, info, id, input=None):
        ...
        exercise_key_instance = ExerciseKey.objects.get(
            exercise=exercise_instance, active=True)
        if exercise_instance and exercise_key_instance:
            ...
            if input.rubric:
                for criterion in input.rubric:
                    ...
                    if criterion.id:
                        try:
                            criterion_instance = Criterion.objects.get(
                                pk=criterion.id, active=True)
                        ...
                        criterion_instance = Criterion(
                            description=criterion.description, exercise_id=exercise_instance.id, points=criterion.points)
                        criterion_instance.save()
            ...
            exercise_instance.save()
            # Updating the points on the chapter
            CreateChapter.update_points(exercise_instance.chapter_id)
...
class DeleteExercise(graphene.Mutation):
    ...
    def mutate(root, info, id):
        ...
        if exercise:
            ...
            try:
                exercise_key = ExerciseKey.objects.all().get(
                    exercise_id=exercise.id, active=True)
            ...
            submissionRubric = CriterionResponse.objects.all().filter(
                exercise_id=exercise.id, active=True)
            ...
            exercise_submissions = ExerciseSubmission.objects.all().filter(
                exercise_id=exercise.id, active=True)
            # Updating the points on the chapter
            CreateChapter.update_points(exercise.chapter_id)
...
class CreateCriterion(graphene.Mutation):
    ...
    def mutate(root, info, input=None):
        ...
        criterion_instance = Criterion(
            description=input.description, exercise_id=input.exercise_id, points=input.points)
        criterion_instance.searchField = CreateCriterion.generate_searchField(
            criterion_instance)
        criterion_instance.save()
...
class DeleteCriterion(graphene.Mutation):
    ...
    def mutate(root, info, id):
        ...
        if criterion:
            ...
            criterion_resposnes = CriterionResponse.objects.all().filter(
                criterion_id=criterion.id, active=True)
...
class CreateCriterionResponse(graphene.Mutation):
    ...
    def mutate(root, info, input=None):
        ...
        criterion_response_instance = CriterionResponse(criterion_id=input.criterion_id, exercise_submission_id=input.exercise_submission_id,
                                                        exercise_id=input.exercise_id, participant_id=input.participant_id, remarker_id=input.remarker_id, score=input.score)
        criterion_response_instance.searchField = CreateCriterionResponse.generate_searchField(
            criterion_response_instance)
...
class UpdateCriterionResponse(graphene.Mutation):
    ...
    def mutate(root, info, id, input=None):
        ok = False
        CreateCriterionResponse.validate_criterion_response_input(input)
        criterion_response_instance = CriterionResponse.objects.get(
            pk=id, active=True)
        ...
        for response in all_criterion_responses:
            try:
                submission = ExerciseSubmission.objects.get(
                    participant_id=response.participant.id, exercise_id=response.exercise.id)
            except:
                print('Could not find submission with participant id ',
                      response.participant.id, ' and exercise id ', response.exercise.id)
...
class PatchExerciseSubmissionsSearchFields(graphene.Mutation):
    ...
    def mutate(root, info, exercise_submissions=None, grading=False, bulkauto=False):
        ...
        for submission in all_submissions:
            ...
            submission.searchField = CreateUpdateExerciseSubmissions.generate_searchField(
                submission)
...
class CreateUpdateExerciseSubmissions(graphene.Mutation):
    ...
    class Arguments:
        exercise_submissions = graphene.List(
            ExerciseSubmissionInput, required=True)
    ...
    def check_errors(exercise_submissions=None, grading=None):
        error = ""
        for submission in exercise_submissions:
            exercise_instance = Exercise.objects.get(
                pk=submission.exercise_id, active=True)
    ...
    def updateCompletedChapter(root, info, chapter_id, participant_id):
        ...
        try:
            ...
            completed_chapter = CompletedChapters.objects.get(
                chapter_id=chapter_id, participant_id=participant_id)
        except:
            pass
        ...
        if chapter and participant and completed_chapter:
            ...
            exerciseCount = Exercise.objects.all().filter(
                chapter_id=chapter_id, active=True).count()
            submittedCount = ExerciseSubmission.objects.all().filter(participant_id=participant_id,
                                                                     chapter_id=chapter_id, status=ExerciseSubmission.StatusChoices.SUBMITTED, active=True).count()
            gradedCount = ExerciseSubmission.objects.all().filter(participant_id=participant_id,
                                                                  chapter_id=chapter_id, status=ExerciseSubmission.StatusChoices.GRADED, active=True).count()
            ...
            for exercise in exercises:
                try:
                    submission = ExerciseSubmission.objects.all().get(
                        participant_id=participant.id, exercise_id=exercise.id, active=True)
            ...
            # Giving them 100% if there are no exercises in the chapter
            percentage = percentage if exerciseCount > 0 else 100
    ...
    def markChapterCompleted(root, info, chapter_id, participant_id):
        ok = False
        chapter = None
        chapter_already_completed = CompletedChapters.objects.filter(
            chapter_id=chapter_id, participant_id=participant_id).exists()
        ...
        if chapter is not None and not chapter_already_completed:
            ...
            required_exercise_ids = Exercise.objects.filter(
                chapter_id=chapter_id, required=True, active=True).values_list('id', flat=True)
            if len(required_exercise_ids) > 0:
                ...
                submitted_exercise_ids = ExerciseSubmission.objects.filter(
                    chapter_id=chapter_id, participant_id=participant_id, active=True).values_list('exercise_id', flat=True)

                # Checking if each of the ids of the required exercise ids list exist in the submitted exercise ids list
                all_required_exercises_submitted = all(
                    item in submitted_exercise_ids for item in required_exercise_ids)
            else:
            ...
            if all_required_exercises_submitted:
                completed_chapter = CompletedChapters(participant_id=participant_id, chapter_id=chapter.id, course_id=chapter.course.id,
                                                      total_points=chapter.points, status=ExerciseSubmission.StatusChoices.SUBMITTED)
                completed_chapter.save()
        ...
        if chapter_already_completed:
            ...
            CreateUpdateExerciseSubmissions.updateCompletedChapter(
                root, info, chapter_id, participant_id)
    ...
    def markCourseCompleted(course_id, participant):
        ...
        if course:
            report = Report.objects.get(
                course_id=course_id, participant_id=participant.id, active=True)
    ...
    def process_submission_rubric(submission, input_rubric):
        ...
        exercise_rubric = Criterion.objects.all().filter(
            exercise_id=exercise.id, active=True).order_by('id')
        submission_rubric = CriterionResponse.objects.all().filter(
            exercise_id=exercise.id, participant_id=submission.participant.id, active=True)
        if exercise_rubric:
            if not submission_rubric:
                ...
                for criterion in exercise_rubric:
                    criterion_response_instance = CriterionResponse(
                        criterion_id=criterion.id, exercise_id=exercise.id, participant_id=submission.participant.id, exercise_submission_id=submission.id, score=0)
                    criterion_response_instance.save()
            else:
                # If it exists already, we simply update them.
                for criterion_response in input_rubric:
                    criterion_response_instance = CriterionResponse.objects.get(
                        criterion_id=criterion_response.criterion_id, participant_id=submission.participant.id, active=True)
    ...
    def process_submission(submission, grading):
        ...
        try:
            exercise_key = ExerciseKey.objects.get(
                exercise_id=exercise.id, active=True)
        except:
            exercise_key = ExerciseKey(
                exercise_id=exercise.id, chapter_id=exercise.chapter.id, course_id=exercise.course.id)
            exercise_key.save()
            pass
        ...
        submission.percentage = submission.points * 100 / \
            totalPoints if totalPoints > 0 else 100  # If total points is 0, then they get 100%
    ...
    def freeze_rubric(submission):
        ...
        try:
            for criterion_response in submission.rubric:
                ...
                criterion_response['criterion'] = {
                    'description': criterion.description, 'points': criterion.points}
                if criterion_response.remarker_id is not None:
                    remarker = User.objects.all().get(pk=int(criterion_response.remarker_id))
                    criterion_response['remarker'] = {
                        'id': remarker.id, 'name': remarker.name}
    ...
    def update_submission(root, info, exercise_submission_instance, grading, autograded, submission, searchField):
        # If it is update, that means it is being graded, so here we add the grader_id
        grader_id = info.context.user.id if grading and not autograded else None
    ...
    def mutate(root, info, exercise_submissions=None, grading=False, bulkauto=False):
        ...
        ok = False
        current_user = info.context.user
        CreateUpdateExerciseSubmissions.check_errors(
            exercise_submissions, grading)  # validating the input
        finalSubmissions = []
        # This is used to track if we need to email the graders so that we only email when there is a non-autograded submission
        ...
        if grading == True and bulkauto == True:
            eligible_exercises = Exercise.objects.filter(question_type__in=[
                                                         Exercise.QuestionTypeChoices.DESCRIPTION, Exercise.QuestionTypeChoices.OPTIONS], active=True)
            exercise_submissions = ExerciseSubmission.objects.filter(
                exercise__in=eligible_exercises, status=ExerciseSubmission.StatusChoices.SUBMITTED, active=True)

        # Looping through the array of submissions to process them individually
        for submission in exercise_submissions:
            ok = True
            autograded = False
            print('Submission in the list ', submission,
                  'current user ', current_user.name)

            # Calculating whether the submission is empty or not (regardless of whether it is for a required exercise)
            empty_submission = CreateUpdateExerciseSubmissions.is_submission_empty(
                submission, submission.exercise_id)

            print('Empty submission or not => ', empty_submission)
            if not empty_submission:

                # Processing the indivdual submission
                processed_submission = CreateUpdateExerciseSubmissions.process_submission(
                    submission, grading)
                submission = processed_submission['submission']
                autograded = processed_submission['autograded']
                if not autograded:
                    # This is used to track if any autograded submission exists
                    manual_grading_required = True

                # Initializing an empty searchField. This will be added before we save it.
                searchField = ''
                ...
                try:
                    ...
                    existing_submission = ExerciseSubmission.objects.get(
                        participant_id=submission.participant_id, exercise_id=submission.exercise_id, active=True)
                ...
                if existing_submission is None:
                    ...
                    exercise_submission_instance = CreateUpdateExerciseSubmissions.update_submission(
                        root, info, exercise_submission_instance, grading, autograded, submission, searchField)

                # Adding additional attributes to searchField
                searchField = CreateUpdateExerciseSubmissions.generate_searchField(
                    exercise_submission_instance)
                exercise_submission_instance.searchField = searchField
                ...
                CreateUpdateExerciseSubmissions.process_submission_rubric(
                    exercise_submission_instance, submission.rubric)

                # Freezing rubric for submission history
                frozen_rubric = CreateUpdateExerciseSubmissions.freeze_rubric(
                    exercise_submission_instance)

                history = SubmissionHistory(exercise_id=exercise_submission_instance.exercise_id, participant_id=exercise_submission_instance.participant_id, option=exercise_submission_instance.option, answer=exercise_submission_instance.answer, link=exercise_submission_instance.link, images=exercise_submission_instance.images,
                                            points=exercise_submission_instance.points, rubric=frozen_rubric, status=exercise_submission_instance.status, flagged=exercise_submission_instance.flagged, grader=exercise_submission_instance.grader, remarks=exercise_submission_instance.remarks, active=exercise_submission_instance.active, searchField=searchField)
                history.save()
                ...
                if grading:
                    ...
                    CreateUpdateExerciseSubmissions.updateCompletedChapter(
                        root, info, exercise_submission_instance.chapter.id, exercise_submission_instance.participant.id)
        ...
        if not grading and finalSubmissions:
            ...
            CreateUpdateExerciseSubmissions.markChapterCompleted(
                root, info, chapter_id, participant_id)
            ...
            if manual_grading_required:
                ...
                CreateUpdateExerciseSubmissions.notify_graders(
                    root, info, exercise_submission_instance)
            ...
            UpdateReport.recalculate(root, info, finalSubmissions)
...
class PatchReportsSearchFields(graphene.Mutation):
    ...
    def mutate(root, info):
        ...
        for report_instance in all_reports:
            ...
            report_instance = UpdateReport.generate_searchfield(
                report_instance)
...
class PatchCompletedChapters(graphene.Mutation):
    ...
    def mutate(root, info):
        ok = True
        count = CompletedChapters.objects.filter(
            status=ExerciseSubmission.StatusChoices.PENDING).count()
        completed_chapters = CompletedChapters.objects.filter(
            status=ExerciseSubmission.StatusChoices.PENDING)
        for chapter in completed_chapters:
            CreateUpdateExerciseSubmissions.updateCompletedChapter(
                root, info, chapter.chapter_id, chapter.participant_id)
...
class UpdateReport(graphene.Mutation):
    ...
    def recalculate(root, info, all_submissions):
        unique_submissions = UpdateReport.remove_duplicate_submissions(
            all_submissions)
    ...
    def mutate(root, info, course_id, participant_id):
        ...
        if report_instance:
            ...
            completed = 0
            course_chapters_count = Chapter.objects.all().filter(
                course_id=course_id, active=True).count()
            completed_chapter_count = CompletedChapters.objects.filter(
                participant_id=participant_id, course_id=course_id).count()
            ...
            CreateUpdateExerciseSubmissions.markCourseCompleted(
                course_id, participant)
