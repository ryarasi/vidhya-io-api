...
INSTITUTION_CACHE= ['public_institutions', 'institutions']
GROUP_CACHE = ['groups','admin_groups']
ANNOUNCEMENT_CACHE = ['announcements', 'public_announcements']
USER_CACHE = ['users', 'public_users']
USER_ROLE_CACHE = ['user_roles']
PROJECT_CACHE = ['projects']
COURSE_CACHE = ['courses']
CHAPTER_CACHE = ['chapters', 'assignments']
EXERCISE_CACHE = ['exercises', 'exercise_keys']
EXERCISE_SUBMISSION_CACHE = ['exercise_submissions', 'submissions_groups']
...
def generate_users_cache_key(searchField=None, all_institutions=None, membership_status_not=None, membership_status_is=None, roles=None, unpaginated=None, limit=None, offset=None):
    ...
    cache_key =  str(item) + separator + searchField_label + str(searchField) + 'all_institutions' + str(all_institutions) + 'membership_status_not' + str(membership_status_not_str) + 'membership_status_is' + str(membership_status_is_str) + 'roles' + str(roles_str) + 'unpaginated' + str(unpaginated) + limit_label + str(limit) + offset_label + str(offset)
    return sanitize_cache_key(cache_key)

def generate_public_users_cache_key(searchField=None, membership_status_not=None, membership_status_is=None, roles=None, limit=None, offset=None):
    item = USER_CACHE[1]
    cache_key =  str(item) + separator + searchField_label + str(searchField) +  'membership_status_not' + str(membership_status_not) + 'membership_status_is' + str(membership_status_is) + 'roles' + str(roles) + limit_label + str(limit) + offset_label + str(offset)
    return sanitize_cache_key(cache_key)

def generate_projects_cache_key(searchField=None, limit=None, offset=None, author_id=None, user=None):
    item = PROJECT_CACHE[0]
...
def generate_submissions_cache_key(searchField=None, limit=None, offset=None, exercise_id=None, chapter_id=None, course_id=None, participant_id=None, submission_id=None, status=None, flagged=None):
    item = EXERCISE_SUBMISSION_CACHE[0]
...
def generate_submission_groups_cache_key(searchField=None, limit=None, offset=None, group_by=None, status=None, flagged=None):
    item = EXERCISE_SUBMISSION_CACHE[1]
...
def invalidate_cache(key):
    try:
        print('cache => ', cache)
        print('cache._cache => ', cache._cache)
        print('cache._cache.keys()', cache._cache.keys())
        cache_keys = list(cache._cache.keys())
        print('cache.keys => ', cache_keys)
        if key and cache_keys:
...
def invalidate_users_cache(user, key):
    try:    
        ...
        if key and cache_keys:
            for item in cache_keys:
                if item.startswith(user.id) and key in item:
                    cache.delete(item)
    except:
        cache.clear()
        print('cache invalidation failed!')
