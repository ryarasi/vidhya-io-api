...
def generate_users_cache_key(entity=None, searchField=None, all_institutions=None, membership_status_not=None, membership_status_is=None, roles=None, unpaginated=None, limit=None, offset=None):
    ...
    cache_key = str(entity) + separator + searchField_label + str(searchField) + 'all_institutions' + str(all_institutions) + 'membership_status_not' + str(membership_status_not_str) + \
...
def generate_public_users_cache_key(entity, searchField=None, membership_status_not=None, membership_status_is=None, roles=None, limit=None, offset=None, user_role=None, user_institution=None):
    cache_key = str(entity) + separator + searchField_label + str(searchField) + 'membership_status_not' + str(membership_status_not) + 'membership_status_is' + str(
        membership_status_is) + 'roles' + str(roles) + limit_label + str(limit) + offset_label + str(offset) + 'role' + str(user_role) + 'institution' + str(user_institution)
    return sanitize_cache_key(cache_key)
...
def generate_groups_cache_key(entity, searchField, limit, offset, current_user):
    cache_key = str(current_user.id) + separator + str(entity) + separator + searchField_label + \
        str(searchField) + separator + limit_label + \
        str(limit) + separator + offset_label + str(offset)
    return sanitize_cache_key(cache_key)
...
def generate_projects_cache_key(entity, searchField=None, sortBy=None, limit=None, offset=None, author_id=None, user=None):
    cache_key = str(user.id) + str(entity) + separator + searchField_label + str(searchField) + 'sortBy' + \
        sortBy + limit_label + str(limit) + offset_label + \
        str(offset) + 'author_id' + str(author_id)
    return sanitize_cache_key(cache_key)


def generate_public_courses_cache_key(entity, searchField, limit, offset):
    cache_key = str(entity) + separator + searchField_label + str(searchField) + \
        separator + limit_label + \
        str(limit) + separator + offset_label + str(offset)
    return sanitize_cache_key(cache_key)
...
def generate_chapters_cache_key(entity, searchField, limit, offset, course_id, current_user):
    cache_key = str(current_user.id) + str(entity) + separator + searchField_label + str(searchField) + \
        limit_label + str(limit) + offset_label + \
        str(offset) + 'course_id' + str(course_id)
    return sanitize_cache_key(cache_key)


def generate_exercises_cache_key(entity, searchField=None, limit=None, offset=None, chapter_id=None, user=None):
    cache_key = str(user.id) + str(entity) + separator + searchField_label + str(searchField) + \
        limit_label + str(limit) + offset_label + \
        str(offset) + 'chapter_id' + str(chapter_id)
    return sanitize_cache_key(cache_key)
...
def generate_exercise_keys_cache_key(entity, searchField=None, limit=None, offset=None, exercise_id=None, chapter_id=None, course_id=None):
    cache_key = str(entity) + separator + searchField_label + str(searchField) + limit_label + str(limit) + offset_label + \
        str(offset) + 'exercise_id' + str(exercise_id) + 'chapter_id' + \
        str(chapter_id) + 'course_id' + str(course_id)
    return sanitize_cache_key(cache_key)
...
def generate_submissions_cache_key(entity, searchField=None, limit=None, offset=None, exercise_id=None, chapter_id=None, course_id=None, participant_id=None, submission_id=None, status=None, flagged=None):
    cache_key = str(entity) + separator + searchField_label + str(searchField) + limit_label + str(limit) + offset_label + str(offset) + 'exercise_id' + str(exercise_id) + 'chapter_id' + \
        str(chapter_id) + 'course_id' + str(course_id) + 'participant_id' + str(participant_id) + \
        'submission_id' + str(submission_id) + status_label + \
        str(status) + 'flagged' + str(flagged)
    return sanitize_cache_key(cache_key)
...
def generate_submission_groups_cache_key(entity, searchField=None, limit=None, offset=None, group_by=None, status=None, flagged=None, cutoff_date=None):
    cache_key = str(entity) + separator + searchField_label + str(searchField) + limit_label + str(limit) + offset_label + str(
        offset) + 'group_by' + str(group_by) + status_label + str(status) + 'flagged' + str(flagged) + 'cutoff_date' + str(cutoff_date)
    return sanitize_cache_key(cache_key)


def generate_assignments_cache_key(entity, limit, offset, status, current_user):
    cache_key = str(current_user.id) + str(entity) + separator + limit_label + \
        str(limit) + offset_label + str(offset) + status_label + str(status)
    return sanitize_cache_key(cache_key)
...
def generate_reports_cache_key(entity, searchField=None, limit=None, offset=None, participant_id=None, course_id=None, institution_id=None, user=None):
    cache_key = str(user.id) + str(entity) + separator + searchField_label + str(searchField) + limit_label + str(limit) + offset_label + \
        str(offset) + 'participant_id' + str(participant_id) + 'course_id' + \
        str(course_id) + 'institution_id' + str(institution_id)
    return sanitize_cache_key(cache_key)
